...
export function useChatManager(defaultChannel: string = 'general') {
  const { user } = useAuth();
  ...
  const addReaction = useCallback(async (messageId: string, reaction: string) => {
    ...
    toast.info(`Reaction ${reaction} added (not really)`);
  }, []);

  // Reply to message
  const replyToMessage = useCallback(async (messageId: string, content: string) => {
    console.log(`Replying to message ${messageId} with: ${content}`);
    toast.info(`Reply sent (not really)`);
    return true;
  }, []);
