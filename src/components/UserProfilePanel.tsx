...
const UserProfilePanel = () => {
  ...
  const [profileData, setProfileData] = useState({
    ...
    bio: user?.profile?.bio || "UI/UX Designer and Developer based in San Francisco. I love creating beautiful and functional interfaces.",
    website: user?.profile?.website || "example.com",
    location: user?.profile?.location || "San Francisco, CA",
    timeZone: user?.profile?.timeZone || "PST (UTC-8)",
  });
  
  // Settings state
  const [settings, setSettings] = useState({
    emailNotifications: user?.preferences?.emailNotifications ?? true,
    desktopNotifications: user?.preferences?.desktopNotifications ?? true,
    soundEnabled: true,
    darkMode: darkMode,
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <Badge variant="outline" className="bg-primary/10 text-primary border-primary/20">
                        ...
                        {user?.createdAt ? `Joined ${new Date(user.createdAt).toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}` : 'Joined April 2023'}
                      </Badge>
                    </div>
                  </div>
                </div>
                      ...
                      <div className="flex flex-wrap gap-3">
                        {["blue", "purple", "green", "orange"].map(theme => (
                          <button
                            ...
                            className={`h-10 w-10 rounded-full bg-${theme}-500 ${settings.colorTheme === theme ? 'ring-2 ring-offset-2 ring-${theme}-500' : ''} transition-all hover:scale-110`}
