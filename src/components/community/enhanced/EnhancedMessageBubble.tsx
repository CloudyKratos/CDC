...
export const EnhancedMessageBubble: React.FC<EnhancedMessageBubbleProps> = ({
  ...
  return (
    <div 
      className={cn(
        "group relative transition-all duration-300 px-4 py-3",
        "hover:bg-slate-50/70 dark:hover:bg-slate-800/30 rounded-xl",
        "touch-manipulation will-change-transform", // Better touch response and performance
      ...
      onTouchEnd={() => {
        // Keep actions visible longer on mobile for better UX
        setTimeout(() => {
    ...
    >
      <div className="flex gap-2 sm:gap-3">
        {/* Enhanced Avatar Column - Mobile responsive */}
        <div className="flex-shrink-0 w-10 sm:w-12">
          {showAvatar ? (
            <Avatar className="h-10 w-10 sm:h-12 sm:w-12 ring-2 ring-white dark:ring-slate-800 shadow-lg touch-target transition-transform hover:scale-105">
              <AvatarImage 
            ...
            <div className="w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center">
              <span className="text-xs text-slate-400 opacity-0 group-hover:opacity-100 transition-all duration-300 hidden sm:block font-medium">
        ...
        {/* Enhanced Message Content - Mobile responsive */}
        <div className="flex-1 min-w-0 max-w-[calc(100vw-8rem)] sm:max-w-none">
          {/* Enhanced Header with name and timestamp */}
          {showAvatar && (
            <div className="flex items-baseline gap-3 mb-2 overflow-hidden">
              ...
              <span className="text-xs text-slate-500 dark:text-slate-400 flex-shrink-0 bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded-full">
                {timeAgo}
              </span>
            </div>
          )}
