...
BEGIN
  IF EXISTS (
    ...
    IF NOT EXISTS (
      ...
      EXECUTE $$
        CREATE TRIGGER decrement_message_thread_count
        AFTER DELETE ON public.community_messages
        FOR EACH ROW EXECUTE FUNCTION public.decrement_thread_count()
      $$;
    END IF;
  END IF;
...
BEGIN
  IF to_regclass('public.message_read_status') IS NOT NULL 
    ...
    IF NOT EXISTS (
      ...
      EXECUTE $$
        CREATE TRIGGER sync_read_position
        AFTER INSERT ON public.message_read_status
        FOR EACH ROW EXECUTE FUNCTION public.sync_channel_read_position()
      $$;
    END IF;
  END IF;
END$$;
...
BEGIN
  IF NOT EXISTS (
    ...
    EXECUTE $$
      CREATE TRIGGER notify_mention_parser_on_insert
      AFTER INSERT ON public.community_messages
      FOR EACH ROW EXECUTE FUNCTION public.notify_mention_parser()
    $$;
  END IF;
END$$;

-- Note on RLS: An INSERT policy for channel membership already exists ("Channel members can send messages"). No additional policy added to avoid duplication.